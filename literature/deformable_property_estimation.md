...
3DGS-MPM 系手法（PhysGaussian・PhysGS・GIC 等）は各ガウシアン核にヤング率・ポアソン比を個別に割り当てるため、空間的不均質性は原理的に表現できる。しかしこれらのパラメータはスカラーであり、等方性材料（全方向に同一応答）を前提とする。繊維強化複合材・生体組織・布のように方向によって剛性が異なる異方性材料を正確に記述するには、4階の弾性テンソル（一般に21独立成分）が必要であり、スカラー2パラメータへの還元は原理的な表現力の欠如である。Zheng2024_DiffCPのAEP モデルは異方性対応を主な貢献として掲げるが計算コストが高く、Scheikl2023_RealtoSimは手術組織の異方性を「捉えられない」と明示した。異方性弾性テンソルを実観測から効率よく同定する手法は未確立のままである。

**Gap 4: 不均質パラメータ推定の計算スケーラビリティ**

空間的に不均質なパラメータ分布（ガウシアン核・有限要素ノード単位での物性推定）は原理的には表現可能だが、推定の計算コストが空間解像度に対してスケールしないという問題がある。Li2024_BayesianDiffPhysicsは変分推論のメモリ・計算コストが高解像度化で破綻することを制限として挙げており、後ろ向きMPMも同様のメモリ問題を抱える。高解像度な不均質パラメータ分布を実用的なコストで推定するスケーラブルな手法は未確立のままである。

**Gap 5: 実ロボット実験でのリアルタイム性と精度の両立**

Zheng2025_PIDGは数時間から数日の計算時間を要し、Scheikl2023_RealtoSimは1ステップあたり0.9–2.6秒かかりリアルタイム制御不能であり、Li2024_BayesianDiffPhysicsは後ろ向きMPMのメモリ要求を制限として挙げている。高速化のためにニューラルダイナミクス（Lin2024_AdaptiGraph・Huang2026_SoMA）やバネ質点（Zheng2025_PhysTwin）が使われるが、物理的解釈可能性と汎化性能が犠牲になる。精度・速度・汎用性の三角形トレードオフは、いかなる現行手法によっても同時には解消されていない。

**Gap 6: 推定精度とロボット制御性能の定量的対応関係**
...
**RQ4**: 複数の論文が繰り返し指摘する未解決課題は以下の通りである。第一に、接触不連続・大変形・破断を伴う場面での微分可能シミュレーションの信頼性が根本的に限界に達しており、いかなる手法も一般的な解決策を提示できていない（Gap 1）。第二に、オクルージョン・自己接触下での3D観測の信頼性が課題であり、現行の点群・3DGSベース手法は重度の遮蔽に脆弱である（Gap 2）。第三に、異方性材料の構成モデルとしてスカラーのヤング率・ポアソン比では表現力が原理的に不足しており、4階弾性テンソルの同定手法が未確立である（Gap 3）。第四に、高解像度な不均質パラメータ推定の計算スケーラビリティが現行手法の制約となっている（Gap 4）。第五に、リアルタイム性と推定精度の両立が現行手法の最大のボトルネックである（Gap 5）。第六に、推定精度と操作性能の定量的対応関係が体系的に未調査であり、エンジニアリング上の設計基準が存在しない（Gap 6）。ArriaolaRios2020が2020年時点で指摘した「新規材料に対するEとνの推定は未解決」という根本的課題が、2026年現在もなお完全には解決されていないことは注目に値する。
