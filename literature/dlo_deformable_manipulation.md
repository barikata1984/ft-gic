...
7. **2024: DER リアルタイム制御とロープ動的操作** — DER（Discrete Elastic Rods）統合 MPC（Model Predictive Control）によるロープ動的操作の研究が進展し、GNN ベース MPC が高周波制御に適用され始めた。
...
|-----------|--------|--------|
| リアルタイム DER（Discrete Elastic Rods）シミュレータ | Available | DER の微分可能実装（DiffTaichi ベース）が存在し、500Hz 程度の MPC ループは計算上実現可能 |
...
| Cat 2: DLO Control & Planning | DLO 整線・結び目・ケーブルルーティングの計画・制御 | 7 |
...
DLO の整線・ケーブルルーティング・結び目形成に対する制御・計画手法を扱う 7 論文。DER や Cosserat ロッドなどの物理モデルを内部モデルとして用いるアプローチから、強化学習・自己教師あり学習による純データドリブン手法まで広範な手法を包含する。
...
- Papers presented to user: 54（重複除去後）
- User additions: 0
- User removals: 0
...
- Removed (unverifiable): 2（最終収録: 54 論文）
...
- Papers with limit recorded: 54 / 54 (100%)
...
3. **RQ3**: MPM（DiffTaichi・PlasticineLab・JAX-MPM）および DER への統合可能なアプローチとして、微分可能 MPC・GNN ダイナミクスモデル・残差ポリシーの 3 系統が有望であることが確認された。DER-MPC は理論的に動的操作への適用が有望な統合路である。
