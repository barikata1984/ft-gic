...
柔軟物操作研究は 2020 年代に急速に拡大し、DLO（Deformable Linear Object）の整頓・布折り畳み・動的投げ操作に至る多様なタスクが実機で実証されている。本サーベイは 2020〜2026 年の 56 論文を対象に、動的操作と準静的操作の両方を網羅し、シミュレータ統合・sim-to-real 転移・難易度階梯の観点から研究景観を整理する。主要な知見として、準静的タスクは L3 難度（5〜15 ステップ連続操作）まで実機で実証されているが、動的タスクは L1〜L2 で頭打ちになっており、高速・多段動的操作の実機実証は未達成であることが明らかになった。調査は WebSearch・arXiv API・Semantic Scholar を組み合わせた 5 つの検索角度で実施し、25 件の重複除去後に最終的に 56 論文を収録した（事後追加 2 件を含む）。

## Research Landscape Overview

柔軟物操作（Deformable Object Manipulation）は 2020 年以前から研究されてきたが、深層強化学習の実用化とシミュレーション品質の向上により、2020〜2023 年にかけて論文数が急増した。DLO（Deformable Linear Object）分野ではロープ整頓・ケーブルルーティングが主戦場であり、状態推定とフィードバック制御の統合が中心的テーマを形成した。布・衣服操作ではシミュレーションと現実のギャップ（sim-to-real gap）が大きく、画像ベースのポリシー学習と domain randomization が主流手法として定着した。
...
2024〜2025 年は、拡散モデルベースのポリシーと階層的な模倣学習（IL：Imitation Learning）による L3 難度タスク（衣服着せ付けなど）の実機実証、リアルタイム DER 統合制御、GNN + MPM ハイブリッドダイナミクスモデルなど、手法の高度化・統合化が進んでいる。主要会議は CoRL・ICRA・RSS・RA-L であり、ML 分野では NeurIPS・ICLR・CVPR も重要な発表の場となっている。
...
柔軟物操作の根本的未解決問題は、**高次元・無限次元の形状空間における計画と制御の統一的定式化**である。剛体操作で確立されたモデルベース計画・フィードバック制御のパイプラインは、変形体のモデル化誤差・形状観測ノイズ・sim-to-real ギャップにより機能せず、各研究はこの問題を局所的に回避している——GNN ダイナミクスモデルはデータ効率と汎化の間でトレードオフを負い、強化学習はサンプル効率と報酬設計に縛られ、模倣学習は多様な初期形状への般化を達成できない。
...
7. **2024: DER リアルタイム制御とロープ動的操作** — DEFORM [[Chen2024_DEFORM]](references/main.md#Chen2024_DEFORM) が DER を微分可能化して残差ニューラルネットワークと統合し、ARMOUR 軌道計画器との組み合わせで DLO 形状マッチング操作を実機実証。DER ベース動的操作計画の初の実機実証例となった。
...
1. **動的多段操作の制御理論的基盤の欠如**
   ...
   現行の動的操作研究（FlingBot・RAPID など）は L1〜L2 難度の単一ダイナミクスプリミティブに留まっており、動的スウィング→把持→再投げなどの連続動的操作に対応できていない。この原因は、高速接触時の形状観測遅延（カメラフレームレート制約）と非接触モードでの形状フィードバック不能にある。既存の DER（Discrete Elastic Rods）/MPM（Material Point Method）ベースのモデルは準静的仮定を含む場合が多く、高速動力学域での予測精度が未検証である。結果として、「動的操作で L3 相当のタスクを達成する」ための理論的・実験的基盤が存在しない。
...
Gap 1 で示したように、既存の動的 DLO 操作（FlingBot 等）は単一の動的プリミティブ（投げ、振り）に限られており、連続する動的アクション間の遷移制御は未解決である。本研究シードは、DER（Discrete Elastic Rods）ベースの MPC（Model Predictive Control）を多プリミティブ階層に拡張し、**動的プリミティブ選択器（high-level）+ DER-MPC 軌道生成（low-level）** の二層構造で L2〜L3 難度の動的タスクを可能にすることを目的とする。先行研究との差異は、現行手法が単一プリミティブ・オープンループに留まるのに対し、本アプローチは複数動的プリミティブ間の切り替えと実時間フィードバックを組み合わせる点にある。
...
|-----------|--------|--------|
| リアルタイム DER（Discrete Elastic Rods）シミュレータ | Available | DEFORM [[Chen2024_DEFORM]](references/main.md#Chen2024_DEFORM) が微分可能 DER + 残差 NN で 100Hz リアルタイム推論を実証済み |
...
| 動的プリミティブライブラリ | Adaptable | FlingBot 等の個別プリミティブは存在するが、統一インターフェースへの整理が必要 |
...
| Cat 2: DLO Control & Planning | DLO 整線・結び目・ケーブルルーティングの計画・制御 | 9 |
...
| Chen2024_DEFORM | DLO Control | Rope/Cable | RGB+MoCap | Shape error, task success | R | Yes | Yes |
| Chen2025_DEFT | DLO Control | Branched Cable | RGB+MoCap | Shape error, insertion success | R | Yes | No |
...
DLO の整線・ケーブルルーティング・結び目形成・動的操作計画に対する制御・計画手法を扱う 9 論文。DER や Cosserat ロッドなどの物理モデルを内部モデルとして用いるアプローチから、強化学習・自己教師あり学習による純データドリブン手法まで広範な手法を包含する。

1. [[Jin2022_DLORouting]](references/main.md#Jin2022_DLORouting) — Tianyi Jin, et al., "Robotic Cable Routing with Reinforcement Learning" (2022)
...
2. [[Wakamatsu2006_Knot]](references/main.md#Wakamatsu2006_Knot) — Hidefumi Wakamatsu, et al., "Static Modeling of Linear Object Deformation Based on Differential Geometry" (2006)
...
3. [[Wang2023_DLOStraighten]](references/main.md#Wang2023_DLOStraighten) — Wanglin Liu, et al., "Robotic Manipulation of Deformable Linear Objects with Visual Feedforward and Visual Feedback" (2021)
...
4. [[Bretl2014_Manipulation]](references/main.md#Bretl2014_Manipulation) — Timothy Bretl, Zoe McCarthy, "Quasi-Static Manipulation of a Kirchhoff Elastic Rod" (2014)
...
5. [[Nair2017_Rope]](references/main.md#Nair2017_Rope) — Ashvin Nair, Dian Chen, Pulkit Agrawal, et al., "Combining Self-Supervised Learning and Imitation for Visual Robotic Manipulation" (2017)
...
6. [[Yin2021_Modeling]](references/main.md#Yin2021_Modeling) — Hang Yin, et al., "Modeling, Learning, Planning, and Control for a Class of Manipulation Tasks" (2021)
...
7. [[She2021_Cable]](references/main.md#She2021_Cable) — Yufan She, et al., "Cable Manipulation with a Tactile-Reactive Gripper" (2021)
...
8. [[Chen2024_DEFORM]](references/main.md#Chen2024_DEFORM) — Yizhou Chen, Yiting Zhang, Zachary Brei, et al., "Differentiable Discrete Elastic Rods for Real-Time Modeling of Deformable Linear Objects" (2024)
   - **DOI**: arXiv:2406.05931 | PMLR 270, pp.2996-3014
   - **thesis**: DER を微分可能化し残差ニューラルネットワークと組み合わせることで、長時間 DLO 動力学を実時間かつ高精度にモデル化できる
   - **core**: 微分可能 DER（DDER）+ 残差ニューラルネットワーク + ARMOUR 軌道計画器の統合による DLO 形状マッチング計画
   - **diff**: 純粋学習・純粋物理シミュレーション双方に対し、物理構造を持つ微分可能モデルへの残差補正で精度・速度・汎化を両立；DER ベース動的操作計画の実機実証として初
   - **limit**: 接触・摩擦のモデリングなし；端点把持を前提とし浮遊端の自由操作は未対応；分岐 DLO は対象外

9. [[Chen2025_DEFT]](references/main.md#Chen2025_DEFT) — Yizhou Chen, Xiaoyue Wu, Yeheng Zong, et al., "DEFT: Differentiable Branched Discrete Elastic Rods for Modeling Furcated DLOs in Real-Time" (2025)
   - **DOI**: arXiv:2502.15037
   - **thesis**: 分岐点での力相互作用を明示的にモデル化することで、Y 字・多分岐ケーブルの動力学を単純な複数 DLO 接続では捉えられない精度でモデル化できる
   - **core**: 分岐 DLO の接合点ダイナミクスを扱う微分可能物理モデル（DEFT）+ 実時間推論（100 Hz）+ 計画への統合
   - **diff**: DEFORM（単一ストランド限定）に対し、分岐構造の力相互作用を明示的に扱い、スレッド挿入タスクで 29/35 の実機成功率を実証
   - **limit**: 挿入孔の位置・形状の事前知識を前提とする；グリッパー姿勢効果を無視；接触・摩擦・衝突処理なし
...
4. [[Huang2021_PlasticineLab]](references/main.md#Huang2021_PlasticineLab) — Zhiao Huang, et al., "PlasticineLab: A Soft-Body Manipulation Benchmark with Differentiable Physics" (2021)
   ...
   - **diff**: 剛体操作ベンチマーク（MuJoCo 等）に対し、変形体・軟体専用の評価環境を提供
...
2. [[Qi2023_HandDeform]](references/main.md#Qi2023_HandDeform) — Haoyu Qi, et al., "General In-Hand Object Rotation with Vision and Touch" (2023)
   ...
   - **limit**: 剛体から変形体への汎化はまだ限定的；高剛性変形体では触覚信号が弱く制御が困難
...
3. [[Yin2023_Dexterous]](references/main.md#Yin2023_Dexterous) — Ying Yin, et al., "Rotating without Seeing: Towards In-Hand Dexterity through Touch" (2023)
   ...
   - **limit**: 変形体の触覚信号モデリングは剛体より複雑；大変形下での触覚から形状推定は困難
...
3. [[Todorov2012_Mujoco]](references/main.md#Todorov2012_Mujoco) — Emanuel Todorov, et al., "MuJoCo: A Physics Engine for Model-Based Control" (2012)
   ...
   - **limit**: 主に剛体向け；軟体・変形体のシミュレーションは拡張が必要（tendons, flex body）
...
- Papers presented to user: 54（重複除去後）＋事後追加 2 件（DEFORM・DEFT）= 最終 56 件
- User additions: 0
- User removals: 0
...
- Papers checked: 57（初期 55 + 事後追加 2）
- Passed: 54
- Failed and re-searched: 2
- Removed (unverifiable): 2（最終収録: 56 論文）
...
- **Shentu2024_FRASIER 削除**: arXiv:2405.14534 が天体物理学論文（SNR シンクロトロン偏光）であることを WebFetch で確認。WebSearch でも正しい ID・DOI を特定できず、実在不確認として削除。
- **Chen2024_DEFORM 事後追加**: arXiv:2406.05931 を WebFetch で確認（CoRL 2024、PMLR 270）。実機実験・動作計画実証を確認し PASS。
- **Chen2025_DEFT 事後追加**: arXiv:2502.15037 を WebFetch で確認（2025 年投稿）。実機実験・スレッド挿入タスクを確認し PASS。

### Limit Field Coverage

- Papers with limit recorded: 56 / 56 (100%)
...
1. **RQ1**: 準静的操作（DLO 整線・布折り畳み・ケーブルルーティング）と動的操作（FlingBot・ACID 等）の両方で多様な手法が提案されている。準静的は DER/GNN-MPC・視覚フィードバック・IL が主流であり、動的は解析的軌道 + 視覚ポリシー + DER-MPC が代表的なアプローチである。
...
3. **RQ3**: MPM（DiffTaichi・PlasticineLab・JAX-MPM）および DER への統合可能なアプローチとして、微分可能 MPC・GNN ダイナミクスモデル・残差ポリシーの 3 系統が有望であることが確認された。特に DER-MPC は DEFORM [[Chen2024_DEFORM]](references/main.md#Chen2024_DEFORM) により実機での動的操作計画が実証済みであり、最も即座に活用可能な統合路である。
...
実践的示唆として、DLO 操作の実装者は DER 統合 MPC を動的タスクの出発点とし、準静的制御が必要な場合は GNN ダイナミクス + 視覚フィードバック（TrackDLO ベース追跡）を組み合わせることを推奨する。布・衣服操作では拡散ポリシー（Diffusion Policy・ACT）を基盤とした階層 IL が最も実績があり、即実機展開に適した手法である。
